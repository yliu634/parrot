...
l2fwd_simple_forward(struct rte_mbuf *m, unsigned portid)
{
...

	{
		...
		dp->lookUp(key, v);
		//std::cout << "key is: " << key << " port:" << v << std::endl;
		dst_port = v;
	}
...
l2fwd_main_loop(void)
{
	...
	while (!force_quit) {
		...
		/*
		 * TX burst queue drain, send out packets in tx queue wiht a timer
		 */
...
main(int argc, char **argv)
{
	...
	/* set up hash table for l2fwd */
	{
		...
		othello_hash_cp_t cp(kvs.size()/0.85,true,kvs,std::vector<uint64_t>(kvs.size(),2));
		std::cout << "total kvs size: " << kvs.size() << std::endl;
		dp = new othello_hash_dp_t(cp);
	}
