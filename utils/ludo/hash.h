...
template<class K>
class CRC8FastHasher {
public:
  uint64_t s;
  uint64_t s0;
  size_t len;
  CRC8FastHasher()
    : s0(0x01), len(sizeof(K)) {
      s = s0;
  }
  explicit CRC8FastHasher(uint64_t _s)
    : s0(_s&0xff), len(sizeof(K)) {
      s = _s;
  }
  ...
  inline uint64_t operator()(const K &k0) const {
    const uint8_t *_p = reinterpret_cast<const uint8_t*>(&k0);
    uint8_t crc = (uint8_t)(s & 0xFF);
    for (size_t i = 0; i < len; i++)
      crc = crc8table[crc ^ *_p++];
    return ((uint64_t) crc) << 62;
  }
};

template<class K>
class CRC64RockSoftHasher {
public:
  uint64_t s;
  uint64_t s0;
  size_t len;
  CRC64RockSoftHasher()
    : s0(0xed32a5f61211f537), len(sizeof(K)) {
      s = s0;
  }
  explicit CRC64RockSoftHasher(uint64_t _s)
    : s0(0xed32a5f61211f537), len(sizeof(K)) {
      s = s0 + _s;
  }
  inline void setSeed(uint64_t _s) {
  ...
  inline uint64_t operator()(const K &k0) const {
    const unsigned char *_p = reinterpret_cast<const unsigned char*>(&k0);
    uint64_t crc = ~s;
    for (size_t i = 0; i < len; i++)
...
public:
  ...
  explicit AbslFastHasher64(uint64_t _s)
    : s(_s) {
  }
  inline void setSeed(uint64_t _s) {
    s = _s;
