 ...
 */
template <typename K, size_t Epsilon = 64, size_t EpsilonRecursive = 4,
...
protected:
  ...
  template <typename, size_t, typename> friend class EliasFanoPGMIndex;
...
protected:
  ...
  static void build(RandomIt first, RandomIt last, size_t epsilon,
                    size_t epsilon_recursive, std::vector<Segment> &segments,
    ...
    auto build_level = [&](auto epsilon, auto in_fun, auto out_fun) {
      auto n_segments =
          internal::make_segmentation_par(last_n, epsilon, in_fun, out_fun);
...
public:
  ...
  PGMIndex(RandomIt first, RandomIt last)
      : n(std::distance(first, last)), first_key(n ? *first : K(0)), segments(),
        levels_offsets() {
...
template <typename K, size_t Epsilon, size_t EpsilonRecursive,
          typename Floating>
struct PGMIndex<K, Epsilon, EpsilonRecursive, Floating>::Segment {
  ...
  explicit Segment(const typename internal::OptimalPiecewiseLinearModel<
