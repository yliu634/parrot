...
#include "./utils/ludo/cuckoo_ht.h"
...
public:
  ...
  ControlPlaneLudo(const vector<K> &keys, uint32_t capacity_ = 64, double kLoadFactor_ = 0.95, bool compact = false)
      : locator(1U, true), nKeys(0), capacity(capacity_), kLoadFactor(kLoadFactor_) {
    assert(!compact);
    
    //uint32_t toInsert = min((uint32_t) min(keys.size(), values.size()), capacity);
    uint32_t toInsert = min((uint32_t) keys.size(), capacity);
    ...
    //  num_buckets_ <<= 1U;
    capacity /= kLoadFactor;
    num_buckets_ = (capacity + kSlotsPerBucket - 1)/kSlotsPerBucket;
    num_buckets_ += 32;
    ...
    if (toInsert) {
      for (int i = 0; i < toInsert; ++i) {
        insert(keys[i], keys[i], false);
        if (i % 1000000 == 0)  std::cout << "inserting keys: " << i << std::endl;
      }
    }
    std::cout << "target load facor is: " << kLoadFactor <<  " and real lf is: " << Loadfactor() << std::endl;
    std::cout << "insert fail number: " << nInsert_fails <<", " << "fail ratio: " << nInsert_fails/(double)keys.size() << std::endl;
  }
  ...
  bool powerSize(uint times) {
    checkIntegrity();
    std::cout << "Dont enter here plz" << std::endl;
...
//    v = v & ValueMask;
    ...
    for (uint32_t &b : buckets) {
      ...
      for (char slot = 0; slot < kSlotsPerBucket; slot++) {
        ...
        if (k == bucket.keys[slot]) {
          ...
          return {0, {{0, b, uint8_t(CRC8FastHasher<K>(bucket.seed)(k) >> 62)}}};
        }
      }
    }
    checkIntegrity();
  ...
  double Loadfactor() {
    return nKeys/(double)(num_buckets_*kSlotsPerBucket);
  }
  ...
  inline bool putItem(const K &k, const V &v, uint32_t b, uint8_t slot, bool toFirstBucket,
    ...
    bucket.occupiedMask |= 1U << slot;
    uint8_t toSlot[4];
    uint8_t seed = updateSeed(b, path ? toSlot : nullptr, slot, succ);
    
    if (!succ) { RemoveInBucket(k, bucket); return false;}
    if (path) {
      ...
      PathEntry entry = {result.status ? -2 + toFirstBucket : (int) cc.size(),
                         b, uint8_t(CRC8FastHasher<K>(seed)(k) >> 62), seed,
  ...
  inline bool moveItem(uint32_t sBkt, uint8_t sSlot, uint32_t dBkt, uint8_t dSlot, vector<PathEntry> *const path = 0) {
    ...
    bool succ(true);
    uint8_t toSlot[4];
    uint8_t seed = updateSeed(dBkt, path ? toSlot : nullptr, dSlot, succ);
    ...
    if (path) {
      ...
      PathEntry entry = {(int) vector.size(),
                         dBkt, uint8_t(CRC8FastHasher<K>(seed)(k) >> 62), seed,
                         toSlot[0], toSlot[1], toSlot[2], toSlot[3]};
  ...
  uint8_t updateSeed(uint32_t bktIdx, uint8_t *dpSlotMove = 0, char slotWithNewKey = -1, bool& succ = true) {
    ...
    for (uint8_t seed = 0; seed < 255; ++seed) {
      *(uint32_t *) occupied = 0U;
      ...
      if (success) {
        ...
        if (withDp) {
          CRC8FastHasher<K> oldH(bucket.seed);
  ...
  Bucket getDpBucket(uint32_t index) const {
    ...
    CRC8FastHasher<K> h(result.seed);
    ...
    while (!cpq_.empty()) {
      ...
      if (free_slot != -1) {
        ...
        while (entry.depth > 1) {
          ...
          bool res = moveItem(parent.bucket, entry.parent_slot, entry.bucket, free_slot, online ? &result.path : nullptr);
