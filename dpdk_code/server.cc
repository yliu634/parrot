...
static unsigned int app_rx_queue_per_lcore = 2;
...
print_stats(void)
{
	...
	for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
		...
		printf("\nStatistics for port %u :"
			   ...
			   portid,
			   port_statistics[portid].tx - port_statistics[portid].prev_tx,
			   port_statistics[portid].rx - port_statistics[portid].prev_rx,
			   port_statistics[portid].dropped - port_statistics[portid].prev_dropped);

		port_statistics[portid].prev_tx = port_statistics[portid].tx;
		port_statistics[portid].prev_rx = port_statistics[portid].rx;
		port_statistics[portid].prev_dropped = port_statistics[portid].dropped;

		total_packets_dropped += port_statistics[portid].dropped;
		total_packets_tx += port_statistics[portid].tx;
		total_packets_rx += port_statistics[portid].rx;
	}
	printf("\nAggregate statistics For the second ================"
...
static void
app_packet_handler(struct rte_mbuf *m, unsigned portid)
{
	unsigned dst_port;
	int sent;
...
app_main_loop(void)
{
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			portid = qconf->rx_port_list[i];
			nb_rx = rte_eth_rx_burst(portid, lcore_id, /* i_queue */
						 pkts_burst, MAX_PKT_BURST);

			for (j = 0; j < nb_rx; j++) {
				m = pkts_burst[j];
				rte_prefetch0(rte_pktmbuf_mtod(m, void *));
				app_packet_handler(m, portid);
        rte_pktmbuf_free(m);
			}
		}
	}
...
static unsigned int
app_parse_nqueue(const char *q_arg)
{
	char *end = NULL;
	unsigned long n;
...
app_parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argvopt, short_options,
		...
		/* nqueue */
		case 'q':
			app_rx_queue_per_lcore = app_parse_nqueue(optarg);
			if (app_rx_queue_per_lcore == 0) {
...
main(int argc, char **argv)
{
	...
	#else
		for (; rx_lcore_id < RTE_MAX_LCORE; rx_lcore_id ++) {
			// Initialize the port/queue configuration of each logical core 
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		ret = rte_eth_dev_configure(portid, nb_lcores, 1, &local_port_conf);
		if (ret < 0)
		...
		/* assgin each lcore with a rx queue (only one port) !! */
		uint32_t nb_rx_queues = nb_lcores;
		for (int i_queue = 0; i_queue < nb_rx_queues; i_queue++){
		ret = rte_eth_rx_queue_setup(portid, i_queue, nb_rxd,
