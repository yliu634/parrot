...
public:
  ...
  explicit CuckooHashTable(std::vector<Key> &keys, uint32_t num_entries, 
                          double kLoadFactor_ = 0.95) {
    entryCount = 0;
    cpq_.reset();
    num_entries /= kLoadFactor_;
    ...
    for (const auto &el : keys) {
      if (insert(el, 2) == nullptr) {
        nFail++;
      }
    }
    std::cout << "Cuckoo insert fail rate: " << nFail << "/" << keys.size() << "=" << nFail/(double)num_entries << std::endl;
  ...
  inline void Update(const Key &k, Value v) {
    for (int i = 0; i < kCandidateBuckets; ++i) {
  ...
  double Loadfactor() {
    return entryCount / (double)(num_buckets_*kSlotsPerBucket);
  }
